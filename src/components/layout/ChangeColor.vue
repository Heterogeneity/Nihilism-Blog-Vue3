...
const hue = ref(260)
const changeColor = () => {
  localStorage.setItem('hue', String(hue.value))
  let getHue = localStorage.getItem('hue')
  hue.value = Number(getHue)
...
onMounted(() => {
  ...
  if (localStorage.getItem('hue')) {
    getHue = localStorage.getItem('hue')
    hue.value = Number(getHue)
  } else {
    changeColor()
  }
...
<template>
  <div>
    <div class="hue-icon" @mousedown="open=!open">
      <svg t="1774881032220" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
           p-id="4889" width="64" height="64">
        <path
            ...
            fill="#ffffff" p-id="4890"></path>
      </svg>
    </div>
    <div class="ab-input" v-show="open" @mouseleave="open=false">
      <div style="display: flex;align-items: center;justify-content: space-between;">
        <div class="hue-button" @click="reset">
          <el-icon>
            <Help/>
          </el-icon>
        </div>
      ...
      <div>
        <input v-model="hue" class="hue" min="0" max="360" step="5" type="range">
      </div>
    </div>
  </div>
</template>
